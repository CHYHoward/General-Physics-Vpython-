...
class CO_molecule:
    ...
    def v_K(self): # return kinetic energy of the vibration motion
        ...
        v_rel_C = self.C.v-self.com_v()
        v_rel_O = self.O.v-self.com_v()
        v_par_C = dot(v_rel_C,self.bond.axis)/(mag(self.bond.axis))
        v_par_O = dot(v_rel_O,self.bond.axis)/(mag(self.bond.axis))
        
        return 0.5*(self.O.m) *(v_par_O)**2+0.5*(self.C.m)*(v_par_C)**2 
    def r_K(self): # return kinetic energy of the rotational motion
        I = self.C.m*(mag(self.C.pos-self.com()))**2 + self.O.m*(mag(self.O.pos-self.com()))**2
        
        v_rel_C = self.C.v-self.com_v()
        v_rel_O = self.O.v-self.com_v()
        L = self.C.m*cross(self.C.pos-self.com(),v_rel_C)+self.O.m*cross(self.O.pos-self.com(),v_rel_O)

        return mag(L)**2/(2*I)
    def com_K(self): #return kinetic energy of the translational motion of the center of mass
        return 0.5*(self.O.m+self.C.m)*(mag(self.com_v()))**2
def collision(a1, a2):
