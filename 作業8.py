...
while True:
    ...
    collision = np.less_equal(p_array,prob)
    the_loop = np.sum(collision)

    theta = 2*pi*np.random.rand(the_loop)
    alpha = 2*pi*np.random.rand(the_loop)

    v_array[collision] = vrms *np.transpose(np.array([np.cos(theta)*np.cos(alpha),np.cos(theta)*np.sin(alpha),np.sin(theta)]))
    ...
    if int(t/dt)%2000 == 0:
        ...
        print("tau = ",tau, "the simulated drift velocity = ",mag(vv/(t/dt)), "theoretical drift velocity = ",q*E*tau/m)
    vd_ball.pos = vv/(t/dt)
